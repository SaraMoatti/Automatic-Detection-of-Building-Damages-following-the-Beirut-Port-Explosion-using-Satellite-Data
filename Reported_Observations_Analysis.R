...
plot(db, combined_datasets_spatial_sub_points, main = "DBSCAN Output", frame = F, axis=Null)
...
dbplot  + coord_equal() + theme(axis.line=element_blank(),
...
autoplot.OpenStreetMap(basemap_bng) + 
  ...
  geom_polygon(data = chulls, 
               aes(coords.x1,coords.x2, 
                   group=dbcluster,
                   fill=dbcluster), 
               alpha = 0.5)  + theme(axis.line=element_blank(),
                                     axis.text.x=element_blank(),
                                     axis.text.y=element_blank(),
                                     axis.ticks=element_blank(),
                                     axis.title.x=element_blank(),
...
dm1 <- tm_shape(points_sf_joined) +
  ...
  tm_legend(show=FALSE)+
  tm_layout(title = "Reported Damages per Operational Zone",title.size = 1,frame=FALSE)



dm1_legend <- tm_shape(points_sf_joined) +
  tm_polygons("density_obs",
              ...
              style = "cont",
              border.col = "DarkGray") +
  tm_layout(legend.only=TRUE, legend.title.size = 1,legend.position=c(-0.2,0.25),asp=0.1)
